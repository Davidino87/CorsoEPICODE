...
function dice() {
  var randomNumber = Math.floor(Math.random() * 6) + 1; // puoi anche fare direttamente return Math.floor(Math.random() * 6) + 1
  return randomNumber;
}
...
var biggerNumber = whoIsBigger(8, 5);
...
var resultArray = splitMe("I love coding");
...
console.log(result1); // Output: "ello"
...
function isThisAnEmail(email) {
  return email.inclues("@") && email.inclues(".");
}
...
var diceRollResult = rollTheDices(3);
...
function howManyDays(pastDate) {
  ...
  const dateParam = new Date(pastDate);

  const differenceInMilliseconds = currentDate - dateParam;
...
var myBirthday = new Date("14-06-1987");
...
var myObject = { name: "Davide", age: 36, city: "Milano" };
var modifiedObject = deleteProp(myObject, "eta");
...
function newestMovie(moviesArray) {
  ...
  var sortedMovies = moviesArray.sort(function (a, b) {
...
console.log("Il numero totale di film è: " + numberOfMovies);
...
function searchByTitle(title) {
  return movies.find((movie) => movie.name === title);
}
...
function removeEremoveIndexlementAtIndex(array, index) {
